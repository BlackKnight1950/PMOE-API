...
app.get('/api/tpra', async (req, res) => {
  ...
  try {
    ...
    // Metrics tuned to the real TPRA columns (Status, CCH Recommendation, Assessment Priority).
    ...
    const isComplete = r2 => /complete/.test(val(r2, statusCol));
    const meta = {
      totalRows:       rows.length,
      statusColumn:    statusCol,
      recColumn:       recCol,
      completedRows:   rows.filter(isComplete).length,
      inProgressRows:  rows.filter(r2 => !isComplete(r2)).length,
      notRecommended:  recCol ? rows.filter(r2 => /not recommended/.test(val(r2, recCol))).length : 0
    };
