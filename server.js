...
// ── Service Request HUB ───────────────────────────────────────────────────────
// Sheet: "Service and PM Support Requests" (Sheet ID 6726741898094468)
// Returns all submitted service/PM support requests with category, status, PM, etc.
const SERVICE_REQUEST_SHEET = '6726741898094468';
...
app.get('/api/service-requests', async (req, res) => {
  ...
  try {
    const r = await fetch(
      `${SS_BASE}/sheets/${SERVICE_REQUEST_SHEET}?pageSize=500`,
      { headers: ssHeaders() }
    );
    if (!r.ok) return res.status(502).json({ error: `Smartsheet ${r.status}` });
    const data = await r.json();
    ...
    const requestIdId       = colId('Request ID');
    const nameId            = colId('Project/Proposal Name');
    const groupId           = colId('Group');
    const reqCategoryId     = colId('Request Category');
    const strategicAreaId   = colId('Strategic Plan Focus Area');
    const sponsorId         = colId('Executive Sponsor');
    const fundedId          = colId('Is this Funded?');
    const outcomesId        = colId('Expected Outcomes (Measurable Intended Results)');
    const scopeId           = colId('Scope of Work (Specific Work That PM will Do)');
    const backgroundId      = colId('Background For Request');
    const submittedById     = colId('Submitted By (CCH email address ONLY)');
    const submissionDateId  = colId('Submission Date');
    const prBoardDecisionId = colId('Project Review Board Decision');
    const directorDecisionId= colId('PMOE Director Decision');
    const directorCommentsId= colId('PMOE Director Comments');
    const projCategoryId    = colId('Project Category');
    const pmId              = colId('Project Manager');
    const statusId          = colId('Status');
    const startId           = colId('Target Start Date');
    const endId             = colId('Target End Date');

    const requests = (data.rows || []).map(row => {
      const cell = id => (row.cells || []).find(c => c.columnId == id);
      const name = cell(nameId)?.displayValue || '';
      if (!name) return null;
      return {
        requestId:        cell(requestIdId)?.displayValue       || '',
        name,
        group:            cell(groupId)?.displayValue           || '',
        requestCategory:  cell(reqCategoryId)?.displayValue     || '',
        projectCategory:  cell(projCategoryId)?.displayValue    || '',
        strategicArea:    cell(strategicAreaId)?.displayValue   || '',
        sponsor:          cell(sponsorId)?.displayValue         || '',
        funded:           cell(fundedId)?.displayValue          || '',
        pm:               normalisePM(cell(pmId)?.displayValue  || ''),
        status:           cell(statusId)?.displayValue          || '',
        prBoardDecision:  cell(prBoardDecisionId)?.displayValue || '',
        directorDecision: cell(directorDecisionId)?.displayValue || '',
        directorComments: cell(directorCommentsId)?.displayValue || '',
        outcomes:         cell(outcomesId)?.displayValue        || '',
        scope:            cell(scopeId)?.displayValue           || '',
        background:       cell(backgroundId)?.displayValue      || '',
        submittedBy:      cell(submittedById)?.displayValue     || '',
        submissionDate:   cell(submissionDateId)?.displayValue  || '',
        targetStart:      cell(startId)?.value                  || '',
        targetEnd:        cell(endId)?.value                    || ''
      };
    }).filter(Boolean);
