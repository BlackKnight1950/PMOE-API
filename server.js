...
app.get('/api/tools', async (req, res) => {
  ...
  try {
    // Fetch sheet rows with objectValue to get cell images (Cover column),
    // and attachment list in parallel
    ...
    // Map rowId → { category, tool, cover }
    // Cover cells are cell images — objectValue returns { objectType:'IMAGE', id, url, ... }
    // Fall back to displayValue/value if Cover is stored as plain text URL instead
    const rowMap = {};
    for (const row of (sheetData.rows || [])) {
      const cell = id => (row.cells || []).find(c => c.columnId == id);
      const coverCell = cell(coverColId);
      // Cell images come back as objectValue.url; plain-text URLs come back as value
      const coverUrl  = coverCell?.objectValue?.url
        || coverCell?.image?.url
        || (typeof coverCell?.value === 'string' && coverCell.value.startsWith('http') ? coverCell.value : '')
        || '';
      rowMap[row.id] = {
        rowId:    String(row.id),
    ...
    // Fetch a fresh pre-signed URL for every attachment in parallel
    ...
    // Also include rows that have NO file attachment (Digital, Ops Research, Statistical, etc.)
    // These show as catalogue entries without a download button
