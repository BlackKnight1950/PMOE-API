    ...
    for (const w of widgets) {
      ...
      if (type === 'METRIC') {
        for (const cd of (c.cellData || [])) {
          ...
          metrics.push({
            title:   title || label || 'Metric',
            label:   label,
            value:   value != null ? value : raw,
            display: disp != null ? String(disp) : (value != null ? String(value) : String(raw))
          });
        }
        continue;
      }
      ...
      if (type === 'CHART') {
        ...
        if (series.length) charts.push({ title, series });
        continue;
      }

      if (type === 'GRIDGANTT') {
        const parsed = parseGridHtml(c.htmlContent);
        if (parsed) tables.push({ title, headers: parsed.headers, rows: parsed.rows });
        continue;
      }
    }
