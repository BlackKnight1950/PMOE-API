...
// ── Dynamic Risk/Issue Log Discovery ─────────────────────────────────────────
// Instead of a hardcoded list (which breaks every time a new project is added),
// we search the Smartsheet workspace for all risk/issue log sheets at startup
// and cache the results. The cache refreshes every 6 hours so new project logs
// appear automatically — zero code changes required.
//
// Discovery uses the Smartsheet Search API, which returns sheet IDs and names.
// We derive the project name from the sheet name using pattern matching.
// Naming conventions seen in the workspace:
//   "<Project> Risks Log"        → risk
//   "<Project> Issues Log"       → issue
//   "Risk Log - <Project>"       → risk
//   "Issue Log - <Project>"      → issue
//   "Risks Log - <Project>"      → risk
//   "Issues Log - <Project>"      → issue

let _riskSheetCache   = [];     // [{ id, project, type }]
let _riskCacheExpiry  = 0;      // epoch ms
const RISK_CACHE_TTL  = 6 * 60 * 60 * 1000; // 6 hours

// Map sheet name → { project, type } by stripping known suffixes/prefixes
function parseRiskSheetName(sheetName) {
  const n = sheetName.trim();

  // Patterns: "<Anything> Risks Log" or "<Anything> Risk Log"
  let m = n.match(/^(.+?)\s+Risks?\s+Log$/i);
  if (m) return { project: m[1].trim(), type: 'risk' };

  // Patterns: "<Anything> Issues Log" or "<Anything> Issue Log"
  m = n.match(/^(.+?)\s+Issues?\s+Log$/i);
  if (m) return { project: m[1].trim(), type: 'issue' };

  // Patterns: "Risk Log - <Anything>" or "Risks Log - <Anything>"
  m = n.match(/^Risks?\s+Log\s*[-–]\s*(.+)$/i);
  if (m) return { project: m[1].trim(), type: 'risk' };

  // Patterns: "Issue Log - <Anything>" or "Issues Log - <Anything>"
  m = n.match(/^Issues?\s+Log\s*[-–]\s*(.+)$/i);
  if (m) return { project: m[1].trim(), type: 'issue' };

  return null;
}

// Normalise parsed project name to match PMOE_LOAD_DATA project names.
// Handles abbreviations like "CCH-Press Ganey" → "CCH-Press Ganey Consumerism",
// "Lab Automation" → "Lab Automation Upgrade", etc.
const PROJECT_NAME_MAP = {
  'CCH-Press Ganey':                        'CCH-Press Ganey Consumerism',
  'Lab Automation':                          'Lab Automation Upgrade',
  'Provident ICU':                           'Provident ICU Closure',
...
// Active project context paths — only these folder paths contain active work
...
async function discoverRiskSheets() {
  if (Date.now() < _riskCacheExpiry && _riskSheetCache.length) {
  ...
  try {
    // Search for all "Risk Log" sheets
    const [riskRes, issueRes] = await Promise.all([
      fetch(`${SS_BASE}/search?query=Risk+Log&scopes=sheetName&pageSize=100`, {
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }
      }),
      fetch(`${SS_BASE}/search?query=Issue+Log&scopes=sheetName&pageSize=100`, {
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }
      })
    ]);
    ...
    for (const result of allResults) {
      if (result.objectType !== 'sheet' && result.objectType !== 'SHEET') continue;
      const id   = String(result.id || result.objectId || '');
      if (!id || seen.has(id)) continue;

      // Only include sheets from active-project folder paths
      const context = (result.contextData?.[0] || result.context || '').toLowerCase();
      const isActive = ACTIVE_CONTEXTS.some(c => context.includes(c));
      if (!isActive) continue;

      // Derive project + type from sheet name
      const parsed = parseRiskSheetName(result.name || result.text || '');
      if (!parsed) continue;

      // Skip template sheets
      if ((result.name || '').toLowerCase().includes('template')) continue;

      seen.add(id);
      sheets.push({
        id,
        project: normProjectName(parsed.project),
        type:    parsed.type
      });
    }

    console.log(`[Risk Discovery] Found ${sheets.length} active risk/issue sheets`);
    sheets.forEach(s => console.log(`  ${s.type.padEnd(5)} ${s.id}  ${s.project}`));

    _riskSheetCache  = sheets;
    _riskCacheExpiry = Date.now() + RISK_CACHE_TTL;
    return sheets;

  } catch (err) {
    console.warn('[Risk Discovery] Search failed:', err.message);
    // Return whatever we have cached (possibly empty on first run)
    return _riskSheetCache;
  }
}
