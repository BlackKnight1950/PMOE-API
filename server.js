...
app.get('/api/tpra', async (req, res) => {
  ...
  try {
    const r = await fetch(`${SS_BASE}/reports/${TPRA_REPORT}?pageSize=500`, { headers: ssHeaders() });
    if (r.status === 401) return res.status(401).json({ error: 'Smartsheet token invalid. Update SMARTSHEET_TOKEN in Render.' });
    if (!r.ok) return res.status(502).json({ error: `Smartsheet error (tpra): ${r.status}` });
    const data = await r.json();

    // Reports expose column metadata via virtualColumnId on each cell.
    const cols = (data.columns || []).map(c => ({ id: c.virtualColumnId ?? c.id, title: c.title }));
    const columns = cols.map(c => c.title);

    const rows = (data.rows || []).map(row => {
      const values = {};
      cols.forEach(c => {
        const cell = (row.cells || []).find(x => (x.virtualColumnId ?? x.columnId) == c.id);
        const v = cell?.displayValue ?? cell?.value ?? '';
    ...
    // Best-effort status roll-up: find a column that looks like a status.
    const statusCol = columns.find(t => /status/i.test(t)) || null;
    const sOf = r2 => statusCol ? (r2.values[statusCol] || '').toLowerCase().trim() : '';
    const meta = {
      totalRows:      rows.length,
      statusColumn:   statusCol,
      inProgressRows: rows.filter(r2 => /in progress|in review|assessing/.test(sOf(r2))).length,
      completedRows:  rows.filter(r2 => /complete|approved|done|closed/.test(sOf(r2))).length,
      notStartedRows: rows.filter(r2 => /not started|intake|new|pending/.test(sOf(r2))).length
    };
