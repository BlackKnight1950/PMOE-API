...
// GET /api/tools — returns all tools from PMOE Tool sheet with fresh S3 download URLs.
// Each attachment URL expires in 60 s, so we fetch them fresh here at request time.
// Cover images use the Smartsheet cell image API: GET /sheets/{id}/rows/{rowId}/cells/{colId}/image
app.get('/api/tools', async (req, res) => {
  if (!process.env.SMARTSHEET_TOKEN) {
    return res.status(500).json({ error: 'SMARTSHEET_TOKEN not configured.' });
  }
  try {
    // Fetch sheet rows and attachment list in parallel
    const [sheetRes, attachListRes] = await Promise.all([
      fetch(`${SS_BASE}/sheets/${TOOLS_SHEET_ID}?pageSize=500`, { headers: ssHeaders() }),
      fetch(`${SS_BASE}/sheets/${TOOLS_SHEET_ID}/attachments?pageSize=500`, { headers: ssHeaders() })
    ]);

    if (!sheetRes.ok)      return res.status(502).json({ error: `Smartsheet sheet ${sheetRes.status}` });
    if (!attachListRes.ok) return res.status(502).json({ error: `Smartsheet attachments ${attachListRes.status}` });

    const [sheetData, attachListData] = await Promise.all([
      sheetRes.json(),
      attachListRes.json()
    ]);
    ...
    const coverColId = cols.find(c => c.title === 'Cover')?.id;

    // Fetch cover image URL for a given row via the Smartsheet cell image API.
    // The endpoint returns a 302 redirect to a temporary S3 URL.
    // We use redirect:'manual' to capture the Location header without downloading the image.
    async function fetchCoverUrl(rowId) {
      if (!coverColId) return '';
      try {
        const r = await fetch(
          `${SS_BASE}/sheets/${TOOLS_SHEET_ID}/rows/${rowId}/cells/${coverColId}/image?redirectMethod=REDIRECT`,
          { headers: ssHeaders(), redirect: 'manual' }
        );
        // 302 → Location header is the S3 image URL
        if (r.status === 302 || r.status === 301) {
          return r.headers.get('location') || '';
        }
        // 200 → some API versions return JSON with a url field
        if (r.status === 200) {
    ...
    // Only row-level file attachments
    const rowAttachments = (attachListData.data || []).filter(a => a.parentType === 'ROW');

    // Unique row IDs across all attachments + all rows (for cover fetching)
    const allRowIds = [...new Set([
      ...rowAttachments.map(a => String(a.parentId)),
      ...Object.keys(rowMap)
    ])];

    // Build a set of row IDs that have at least one file attachment
    const attachedRowIds = new Set(rowAttachments.map(a => String(a.parentId)));

    // Fetch cover images and attachment URLs concurrently
    const [coverMap, toolsWithFiles] = await Promise.all([

      // 1. Cover images — one request per unique row
      Promise.all(allRowIds.map(async rowId => {
        const url = await fetchCoverUrl(rowId);
        return [rowId, url];
      })).then(pairs => Object.fromEntries(pairs)),

      // 2. File attachment fresh download URLs
      Promise.all(
        rowAttachments.map(async a => {
          try {
            const r = await fetch(
              `${SS_BASE}/sheets/${TOOLS_SHEET_ID}/attachments/${a.id}`,
              { headers: ssHeaders() }
            );
            if (!r.ok) return null;
            ...
            return {
              ...
              tool:     row.tool     || '',
              cover:    '',           // filled in below from coverMap
              fileName: a.name       || '',
              mimeType: a.mimeType   || '',
    ...
    // Inject cover URLs into file-backed tools
    toolsWithFiles.forEach(t => { t.cover = coverMap[t.rowId] || ''; });

    // Rows with NO file attachment (Digital, Ops Research, Statistical, etc.)
    ...
    const tools = [...toolsWithFiles, ...toolsNoFiles];
    res.json({ fetchedAt: new Date().toISOString(), tools });
...
// Returns a fresh pre-signed S3 URL for a single attachment (URLs expire ~60 s).
// The frontend calls this on every Download click to guarantee a live URL.
app.get('/api/tool-download', async (req, res) => {
  ...
  try {
    const r = await fetch(
      `${SS_BASE}/sheets/${TOOLS_SHEET_ID}/attachments/${id}`,
      { headers: ssHeaders() }
    );
    if (!r.ok) return res.status(502).json({ error: `Smartsheet ${r.status}` });
    const data = await r.json();
...
// Proxies the Smartsheet cell image for the Cover column of a given row.
// The image API returns a 302 redirect to a temporary S3 URL — we follow it
// and stream the image bytes back to the client so the browser can display it
// without needing a Smartsheet session.
const COVER_COL_ID = '1045942156693380';
app.get('/api/cover-image', async (req, res) => {
  ...
  try {
    // Step 1: get the redirect URL from Smartsheet image endpoint
    const r1 = await fetch(
      `${SS_BASE}/sheets/${TOOLS_SHEET_ID}/rows/${rowId}/cells/${COVER_COL_ID}/image?redirectMethod=REDIRECT`,
      { headers: ssHeaders(), redirect: 'manual' }
    );

    let imageUrl = '';
    if (r1.status === 302 || r1.status === 301) {
      imageUrl = r1.headers.get('location') || '';
    } else if (r1.status === 200) {
      const d = await r1.json().catch(() => ({}));
      imageUrl = d.url || '';
    }

    if (!imageUrl) return res.status(404).send('No cover image');

    // Step 2: fetch the actual image bytes and stream to client
    const r2 = await fetch(imageUrl);
    if (!r2.ok) return res.status(502).send('Image fetch failed');
