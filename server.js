...
app.get('/api/tpra', async (req, res) => {
  ...
  try {
    ...
    const rows = (data.rows || []).map(row => {
      ...
      if (columns.every(t => !values[t])) return null; // skip fully empty rows
      return { rowId: String(row.id), values };
    }).filter(Boolean);
    ...
    const meta = {
      ...
      recColumn:      recCol,
      completedRows:  rows.filter(r2 => val(r2, statusCol) === 'complete').length,
      inProgressRows: rows.filter(r2 => val(r2, statusCol) === 'in progress').length
    };

    res.json({
      fetchedAt: new Date().toISOString(),
      active: { columns, rows, reportId: TPRA_REPORT },
      meta
    });
  } catch (err) {
