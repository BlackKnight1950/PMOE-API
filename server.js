...
const COVER_COLUMN_ID = '1045942156693380'; // Cover column

// Helper: fetch the pre-signed image URL for a cell image using Smartsheet /imageurl endpoint
async function fetchCellImageUrl(rowId) {
  try {
    const r = await fetch(
      `${SS_BASE}/sheets/${TOOLS_SHEET_ID}/rows/${rowId}/cells/${COVER_COLUMN_ID}/imageurl`,
      { headers: ssHeaders() }
    );
    if (!r.ok) return '';
    const data = await r.json();
...
// Proxies a Smartsheet cell image to the browser — the browser cannot call Smartsheet
// directly because the image URL requires an auth token and expires quickly.
app.get('/api/cover-image', async (req, res) => {
  ...
  try {
    const imgUrl = await fetchCellImageUrl(rowId);
    if (!imgUrl) return res.status(404).send('No cover image');
...
app.get('/api/tools', async (req, res) => {
  ...
  try {
    ...
    const toolsNoFiles = Object.values(rowMap)
      ...
      .map(row => ({
        id: `row-${row.rowId}`, rowId: row.rowId,
        category: row.category, tool: row.tool,
        fileName: '', mimeType: '', sizeKb: 0, url: null
      }));
